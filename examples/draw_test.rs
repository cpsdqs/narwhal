...
fn main() {
    let mut app = App::init(
        "Narwhal Drawing Test",
        (0, 1, 0),
        |event, app| match event {
            AppEvent::Ready => {
                let mut win = app.create_window(400, 400, |event, win| {
                    ...
                    {
                        let data: &mut WinData =
                            win.data.downcast_mut().expect("Invalid window data");
                        ...
                        match event {
                            WindowEvent::Ready => {
                                ...
                                composite
                                    .set(defs::CompositeProps::In.into(), Vec::<Drawable>::new());
                            ...
                            WindowEvent::Resized(..) => {
                                ...
                                cam.set(
                                    ...
                                    Matrix4::from_translation(
                                        (0., 0., win_size.y as f64 / 2.).into(),
                                    ),
                                );
                                do_render = true;
                            }
                            WindowEvent::Scheduled => {
                                do_render = true;
                            }
                            WindowEvent::UIEvent(event) => match event.event_type {
                                ...
                                EventType::PointerDragged => {
                                    ...
                                    let delta_len =
                                        (Vector2::new(x, y) - data.prev_point.unwrap()).magnitude();
                                    data.prev_point = Some(Vector2::new(x, y));
                                        ...
                                        .unwrap()
                                    {
                                        Value::Drawables(drawables) => {
                                            ...
                                            let (weight, ..) =
                                                stroke.shape.stroke.as_mut().unwrap();
                    ...
                    if schedule_cb {
                        win.schedule_callback(Duration::new(0, 8_000_000));
                    }
                });
                ...
                if let Some(profile) = win.icc_profile() {
                    let profile = Profile::deser(&mut io::Cursor::new(profile))
                        .expect("Failed to deser profile");
                    presenter
                        .set_profile(profile)
